...
function isAiApiPrefix(prefix: string | null): boolean {
  return prefix !== null && AI_API_PREFIXES.has(prefix);
}

function isStreamingRequest(request: Request): boolean {
  const accept = request.headers.get('accept') || '';
  return accept.toLowerCase().includes(EVENT_STREAM_CONTENT_TYPE);
}
    ...
    try {
      ...
      if (!aiApiRequest) {
        ...
        if (referer) {
          ...
          // 如果没有 referer，添加一个目标域名的 referer
          proxyHeaders.set('referer', `${targetUrl.protocol}//${targetUrl.host}/`);
        }
      }

      proxyRequestInit.headers = proxyHeaders;
      ...
      // 处理需要内容替换的资源类型
      const needsRewrite = !streamingResponse && (
